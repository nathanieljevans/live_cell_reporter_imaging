...
# input reporter color # & reporter translocation
rc1_input = ""
rc2_input = ""
rc1 = ""
rc2 = ""
...
os.mkdir(f"{source_path}/output_t0norm")
output_path = f"{source_path}/output_t0norm"
os.mkdir(f"{output_path}/quality_control")
...
for rc in rc_list:
    os.mkdir(f'{output_path}/{rc}')
    for folder in folder_list:
        os.mkdir(f'{output_path}/{rc}/{folder}')
    os.mkdir(f'{output_path}/{rc}/heatmap/sorted_by_div')
    ...
    # 2) add "cn_ratio" ,
    # 3) delete rows with "cn_ratio" > 95 percentile
    ...
    # 2) backfill maximum 3 nan & then delete rows with > 3 remaining nan & rows with nan value at t=0.0 after backfill
    ...
    def df_to_hmap_nuc_only(dataframe, values, data_identity):
        ...
        dataframe_matrix.fillna(method="backfill", limit=3, inplace=True, axis=1)
        before_drop = dataframe_matrix.shape[0]
        dataframe_matrix.dropna(thresh=(dataframe_matrix.shape[1] - 6), axis=0, inplace=True)
    ...
    # add columns to sort for reporter activity: "above_thres" & "median"
    ...
    def hmap_sort(dataframe):
        ...
        dataframe2.sort_values(by=["median"], ascending=False, inplace=True)
    ...
    def groups(dataframe):
        df_groups = dataframe.groupby("cell__treatment", sort=True)
        return df_groups
    ...
    def heatmap(df_groups, channel, df_open, df_end, colors, minimum, maximum, data_identifier, destination):
        ...
        for name, group in df_groups:
            ...
            # ax = fig.add_subplot(1, 1, i) # row, column, position
            ...
            ax = sns.heatmap(dataframe, cmap=cmap, vmin=minimum, vmax=maximum, yticklabels=False)
            ax.set_title(f"{title}\nn={total_cell_number}", fontsize=60)
            fig.savefig(f"{output_path}/{channel}/{destination}/{title}_{data_identifier}.png")
            plt.close(fig)
            # i+=1
        return
    ...
    def reporter_lineplot(df_groups, channel):
        ...
        for name, group in df_groups:
            ...
            for name2, everything in df_groups:
                dataframe = everything.iloc[:, 0:everything.shape[1] - 4].mean(axis=0)
                sns.lineplot(data=dataframe, color="grey", lw=1)
                sns.despine()
            single_df = group.iloc[:, 0:group.shape[1] - 4].mean(axis=0)
            ax = sns.lineplot(data=single_df, color=cmap(i), lw=12)
            ax.spines['left'].set_linewidth(3)
            ax.spines['bottom'].set_linewidth(3)
            sns.despine()
            # ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
            ax.set(xlabel='time_h', ylabel='mean')
            ax.xaxis.set_tick_params(width=3)
            ax.yaxis.set_tick_params(width=3)
            title = name.replace(f"{cell_reporter}__", "")
            total_cell_number = group.shape[0]
    ...
    heatmap(hmap_rc_groups, rc, df_open=0, df_end=4, colors="PuOr_r", data_identifier="reporter", minimum=0, maximum=1, destination="heatmap")
    ...
    heatmap(hmap_rc_div_groups, rc, colors=palette, df_open=1, df_end=0, minimum=0, maximum=1, data_identifier="div", destination="heatmap")
    heatmap(hmap_rc_n_size_groups, rc, colors=palette, df_open=1, df_end=0, minimum=0, maximum=1, data_identifier="n_size", destination="heatmap")
    ...
    def hmap_div_sort_by_div(df_div, values):
        ...
        div_matrix.sort_values(by=["div"], ascending=False, inplace=True)
        return div_matrix
    ...
    def hmap_reporter_sort_by_div(hmap_div_sorted, hmap_sorted, sort_side):
        ...
        hmap_div_merge.sort_values(by=["div", "median"], ascending=False, inplace=True)
        return hmap_div_merge
    ...
    def reporter_catplot(dataframe, channel):
        ...
        sns.catplot(ax=ax, kind="swarm", data=dataframe, x="treatment", hue="treatment", y="pos_vs_pre", legend_out=True)
        ax.spines['left'].set_linewidth(3)
        ax.spines['bottom'].set_linewidth(3)
    ...
    heatmap(hmap_rc_div_sorted_groups, rc, df_open=1, df_end=4, colors="PuOr_r", data_identifier="reporter", minimum=0, maximum=1, destination="heatmap//sorted_by_div")
    ...
    heatmap(hmap_div_all_sorted_groups, rc, colors=palette, df_open=1, df_end=1, minimum=0, maximum=1, data_identifier="div", destination="heatmap//sorted_by_div")
