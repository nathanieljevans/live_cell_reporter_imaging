...
output_path = f"{source_path}/output_sbs"
...
for rc in rc_list:
    ...
    def df_to_hmap_nuc_only(dataframe, values, data_identity):
        ...
        dataframe_matrix.interpolate(method="linear", axis=1, limit=fill_limits, limit_direction="backward",
                                     inplace=True)
    ...
    def hmap_sort(dataframe):
        ...
        dataframe2["start"] = dataframe_num.iloc[:, 0:1].mean(axis=1)
        ...
        dataframe2.sort_values(by=["treatment", "start", "median"], ascending=[True, False, False], inplace=True)
    ...
    def reporter_lineplot(df_groups, channel):
        ...
        for name, group in df_groups:
            ...
            for name2, everything in df_groups:
                dataframe = everything.iloc[:, 0:everything.shape[1] - 4].mean(axis=0, skipna=True)
                sns.lineplot(data=dataframe, color="grey", lw=1)
                sns.despine()
            single_df = group.iloc[:, 0:group.shape[1] - 4].mean(axis=0, skipna=True)
