...
_repeats_="1" 
_nclus_="5 10 15 20"                    # 2-50
_resample_sz_="50 100 150"              # 50 - 150 
_load_="raw normalized"                 # normalized raw
_burnin_="0 4 8"                        # these are in units of measurement time points so multiply by measurement 
        ...
        for nclus in $_nclus_; do
            ...
            for burnin in $_burnin_; do
                ...
                for resample_sz in $_resample_sz_; do
                    echo -e '\t\t' $resample_sz
                    python HER2_classifier.py --data $skbr3_data --drug neratinib --sensitive_line WT --resistant_line T798I --load $load --nclus $nclus --out $output/SKBR3_NERATINIB/ --resample_sz $resample_sz --burnin $burnin
                done
            done
        done 
    done
    ...
    for load in $_load_; do
        ...
        for nclus in $_nclus_; do
            ...
            for burnin in $_burnin_; do
                ...
                for resample_sz in $_resample_sz_; do
                    echo -e '\t\t' $resample_sz
                    python HER2_classifier.py --data $skbr3_data --drug trastuzumab --sensitive_line WT --resistant_line ND611 --load $load --nclus $nclus --out $output/SKBR3_TRASTUZUMAB/ --resample_sz $resample_sz --burnin $burnin
                done
            done
        done 
    done
    ...
    for load in $_load_; do
        ...
        for nclus in $_nclus_; do
            ...
            for burnin in $_burnin_; do
                ...
                for resample_sz in $_resample_sz_; do
                    echo -e '\t\t' $resample_sz
                    python HER2_classifier.py --data $efm192A_data --drug neratinib --sensitive_line WT --resistant_line T798I --load $load --nclus $nclus --out $output/EFM192A_NERATINIB/ --resample_sz $resample_sz --burnin $burnin
                done
            done
        done 
    done
    ...
    for load in $_load_; do
        ...
        for nclus in $_nclus_; do
            ...
            for burnin in $_burnin_; do
                ...
                for resample_sz in $_resample_sz_; do
                    echo -e '\t\t' $resample_sz
                    python HER2_classifier.py --data $efm192A_data --drug trastuzumab --sensitive_line WT --resistant_line ND611 --load $load --nclus $nclus --out $output/EFM192A_TRASTUZUMAB/ --resample_sz $resample_sz --burnin $burnin
                done
            done
        done 
    done
